...
class towxml {
    ...
    let re = /<[^<]*>/ig,
      ...
      if (isCloseLabel) {
        ...
        ;
          ...
          let delWordBbrackets = word.substr(1, word.length - 2),
              //剔除首尾尖括号
          wordSplit = delWordBbrackets.split(' '),
              //得到元素标签与属性
          labelName = wordSplit[0].toLowerCase(),
              //取得tagName
  ...
  toJson(content, type, app) {
    ...
    if (app) {
      ['bind:touchstart', 'bind:touchmove', 'bind:touchcancel', 'bind:touchend', 'bind:tap', 'bind:longpress', 'bind:longtap', 'bind:transitionend', 'bind:animationstart', 'bind:animationiteration', 'bind:animationend', 'bind:touchforcechange'].forEach(item => {
