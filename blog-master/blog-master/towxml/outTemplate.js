...
class outwxml {
  ...
  outtag(id) {
    ...
    wxmlTag.forEach((item, index) => {
      ...
      ;
      s += `<${item} wx:if="{{item.node === 'element' && item.tag === '${item}'}}" ${attr} ${imgMode}><block wx:for="{{item.child}}" wx:key="{{item}}"><template is="m${id}" data="{{item}}"/></block></${item}>`; // s+= `
  ...
  outattr() {
    ...
    let s = '',
        attr = ['class', 'width', 'height', 'data', 'src', 'id', 'style', 'href', 'checked', 'bind:touchstart', 'bind:touchmove', 'bind:touchcancel', 'bind:touchend', 'bind:tap', 'bind:longpress', 'bind:longtap', 'bind:transitionend', 'bind:animationstart', 'bind:animationiteration', 'bind:animationend', 'bind:touchforcechange'];
    s += `data-_el="{{item}}"`;
  ...
  outwxml() {
    ...
    for (let i = 0, len = _ts.config.depth; i < len; i++) {
      let c = i < len - 1 ? i + 1 : i;
      let temp = `<template name="m${i}"><block wx:if="{{item.node === 'text'}}">{{item.text}}</block>${_ts.outtag(c)}</template>`; // let temp = `<template name="m${i}">
      //         <!--文字-->
